...
"""
successor(zero): lambda f: lambda x: f(zero(f)(x)) which is f(x)
We change the parameter to remove a lambda
because the parameter of `successor(n)` is not related to the result
Think: Why this is reasonable?
My thought: The lambda parameters here are also can be the parameters of the function.
The function signature cal also be
def one(f,x):
    return f(x)
It will get the same result. You can find relate info with keyword curry function.
"""
def one(f):
...
"""
I'm here to calculate the depth of `f(x)`.
so, I need a linear ascent function to record the value.
and the last callback will be f(f(f(...(0))))
"""
def church_to_int(n):
    ...
    3
    """
    return n(lambda t: t+1)(0)

"""
The reason we need lambda x is so that m and n can get two parameters.
Make the church_to_int get valid parameter
"""
def add_church(m, n):
    ...
    5
    """
    return lambda x: lambda f: m(x)(f)+n(x)(f)
