...
def pingpong(n):
    ...
    True
    """
    def cal(now,QAQ,depth):
        if(depth==n):
            return now
        if depth%6==0 or [c for c in str(depth)].count('6')!=0:
            return cal(now-QAQ,-QAQ,depth+1)
        else:
            return cal(now+QAQ,QAQ,depth+1)
...
def missing_digits(n):
    ...
    def rcal(now,past):
        ...
        if now==0:
            return 0
        if now%10==past:
            return rcal(now//10,past)
        return past-now%10-1+rcal(now//10,now%10)
...
def count_change(total, next_money):
    ...
    def cal(left,now):
        if left==0:
            return 1
        elif now is None or left-now<0:
            return 0
        return cal(left-now,now)+cal(left,next_money(now))
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda x: (lambda f,a: 1 if a==1 else a*f(f,a-1))(lambda f,a: 1 if a==1 else a*f(f,a-1),x)
